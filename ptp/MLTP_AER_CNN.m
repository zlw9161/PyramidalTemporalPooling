...
% Function : run AER task using MLTP method with CNN FC features

function MLTP_AER_CNN()
...
Options.EncNonLin = 8;   % Non-Linearity Type before encoding
Options.DataNonLin = 4;   % Non-Linearity Type for encoded data (SVM-Kernel)
Options.PCA = 0;
...
%featfiles = {'dev_ny_b_ep10_25%ovlp','ldrbrd_ny_b_ep10_25%ovlp'};
featfiles = {'train_spct_b_relu_p62_ep13_ovlp','test_spct_b_relu_p62_ep13_ovlp'};
rsltfile = sprintf('aer_results/fow/rslt%s_win3_ser_C0.01_2lftp_ser_chi2.mat',featfiles{1}(6:end));
for f = 1 : numel(featfiles)
    encfile = sprintf('D:/Experiments/mltp/encoded_feats/aer/%s_win3_ser_C0.01_2lftp_ser.mat',featfiles{f});
    if exist(encfile,'file') ~= 2
        featfile = sprintf('D:/Experiments/cnn_feats/ae_cnn/fc_feat/%s.mat',featfiles{f});
        disp(featfile);
        load(featfile);
...
% load label files
load('D:/Experiments/cnn_feats/ae_cnn/fc_feat/train_label.mat');
TrainClass = label;
n_Trn = size(label,1);
clear label;
load('D:/Experiments/cnn_feats/ae_cnn/fc_feat/test_label.mat');
TestClass = label;
n_Test = size(label,1);
clear label;
...
save(rsltfile,'accuracy','precision','recall','F','avg_acc','multiclass');
...
function [EncodedData] = getEncodedData(feats, net, EnonLin)
    ...
    EncodedData = zeros(TOTAL,1024*2*4);
    for i = 1 : TOTAL
...
function [precision, recall, acc, C] = train_and_classify(TrainData_Kern,TestData_Kern,TrainClass,TestClass)
        ...
        %C = 0.01 : 0.01 : 0.5;
        %C = [0.001 0.01 0.1 1 5 10 25 50 75 100 500 1000];
        C = 100:50:1000;
        %C = 1 : 10 : 100;
        CoreNum = 4;
        ...
        for ci = 1 : numel(C)
            % precomputed kernel c-svm
            model = svmtrain(double(TrainClass), double(TrainData_Kern), sprintf('-t 4 -c %1.6f -q -b 1 -m 500',C(ci)));
            %model = svmtrain(TrainClass, TrainData_Kern, sprintf('-t 2 -c %1.6f -g %1.6f -q -e 0.0001',C(ci),gamma(gi)));
            [predicted, acc, scores{ci}] = svmpredict(double(TestClass), double(TestData_Kern) ,model);
            [precision(ci,:) , recall(ci,:)] = perclass_precision_recall(TestClass,predicted);
            accuracy(ci) = acc(1,1);
        end        
        prdctfile = sprintf('kaggle/predicted-dcase-ny-b-ep18-mltp-svm-c0.03.mat');
